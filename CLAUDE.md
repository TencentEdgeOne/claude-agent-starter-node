...
A full-stack Claude Agent starter template running on EdgeOne Pages. The frontend is a React chat UI that communicates with backend "agents" (EdgeOne Pages Functions) via SSE streaming. The backend uses the Claude Agent SDK to invoke Claude with MCP tools in a sandboxed environment.
...
| File | Route | Purpose |
|------|-------|---------|
| `agents/chat/index.ts` | POST /chat | SSE streaming chat via `query()` from Claude Agent SDK |
...
- **Conversation ID**: Passed via `pages-agent-conversation-id` HTTP header for `/chat` and `/history`. For `/stop`, the target conversation ID is in the request body (never in headers — this is critical to avoid signal collision).
...
Configured via `edgeone.json`. Build output goes to `dist/`. Agent timeout is 900s. Framework is `claude-sdk`.
